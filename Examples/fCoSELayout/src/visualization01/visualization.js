...
export function visualization (config) {
  ...
  const cy = cytoscape({
    ...
    ready: function () {
      ...
      //   stop: () => {} // on layoutstop
      // };
      for (let key in this.nodes()[0]) {
        console.log(JSON.stringify("Node[0] " + key))
      }
      console.log(JSON.stringify(this.nodes()[0].attr()))
  ...
  cy.on('mouseover', 'node', function (evt) {
    const node = evt.target
    console.log('mouseover node ' + node.id())
  })

  cy.on('mouseover', 'edge', function (evt) {
    const edge = evt.target
    console.log('mouseover edge ' + edge.id())
  })
